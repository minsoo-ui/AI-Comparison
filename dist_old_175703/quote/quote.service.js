...
let QuoteService = QuoteService_1 = class QuoteService {
    ...
    constructor(uploadService, ocrService, extractService, specialTermsService, aiService) {
    ...
    async compareQuotes(filePaths) {
        this.logger.log(`Comparing ${filePaths.length} quotes...`);
        const rawTexts = await Promise.all(filePaths.map(async (filePath) => {
        ...
        const structuredQuotes = await Promise.all(quoteItems.map(async (item) => {
        ...
        const quoteTextsBlock = quoteItems.map((item, i) => `=== BÁO GIÁ #${i + 1}: "${item.fileName}" ===\n${item.text}\n=== KẾT THÚC BÁO GIÁ #${i + 1} ===`).join('\n\n');
    ...
    buildAnalysisPrompt(quoteTexts, terms, rfqContext, quoteCount) {
        return `/no_think
...
`;
    ...
    async chatWithQuotesContext(message, history, context) {
        ...
        const cleanHistory = history
            ...
            .slice(-6);
        const recentHistory = cleanHistory.map(m => `${m.role === 'user' ? 'User' : 'Assistant'}: ${m.content}`).join('\n');
