...
export class OcrService {
    ...
    return new Promise<OcrResult>((resolve, reject) => {
      ...
      proc.on('close', async (code) => {
        ...
        try {
          ...
          fs.remove(outDir).catch((e) =>
            this.logger.warn(`Failed to cleanup OCR outDir: ${outDir}`, e),
          );
        }
      });
    });
          ...
          pages: [
            {
              pageNumber: 1,
              text: result.text,
              blocks: this.expandBlocks(result),
              raw: result,
            },
          ],
        };
  ...
  private mapSingleBlock(item: any): OcrBlock {
    ...
    const text =
      item.text || (Array.isArray(item[1]) ? item[1][0] : item[1]) || '';
