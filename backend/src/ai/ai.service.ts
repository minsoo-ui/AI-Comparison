...
export class AiService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.llm = new ChatOllama({
      ...
      numPredict: 512, // Prevent token bloat for simple structured JSON endpoints
    });
  ...
  /**
   * Chat method using structured output with Zod schema
   */
  async chatWithStructuredOutput(prompt: string, schema: any): Promise<any> {
    try {
      // Re-instantiate LLM for structured output to ensure clean state
      const structuredLlm = this.llm.withStructuredOutput(schema);
      const response = await structuredLlm.invoke(prompt);
      return response;
    } catch (error) {
      this.logger.error(`Structured LLM call failed: ${error.message}`);
      // Return minimal fallback matching typical schema roots if needed
      return null;
    }
  }
