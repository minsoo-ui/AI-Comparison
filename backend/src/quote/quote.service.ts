...
export class QuoteService {
    ...
    async compareQuotes(filePaths: string[]): Promise<any> {
        this.logger.log(`[Map-Reduce] Starting comparison for ${filePaths.length} files...`);
    ...
    private async mapProcessFile(filePath: string) {
        ...
        this.logger.log(`OCR complete for ${fileName}, text length: ${text.length}`);
        ...
        if (type === 'quote') {
            ...
            // Dùng text từ ocrResult
            const extraction = await this.extractService.extractData(text, quoteSchema);
    ...
    private classifyFile(fileName: string, text: string): 'quote' | 'rfq' | 'terms' {
        const lowerText = (fileName + ' ' + text).toLowerCase();
        if (lowerText.includes('common') || lowerText.includes('term') || lowerText.includes('thuật ngữ')) {
            return 'terms';
        } else if (lowerText.includes('hỏi cước') || lowerText.includes('thông tin') || lowerText.includes('rfq') || lowerText.includes('request')) {
            return 'rfq';
        }
        return 'quote';
    }
    ...
    private async reduceResults(mapResults: any[]) {
        this.logger.log(`[Reduce] Aggregating results from ${mapResults.length} maps...`);

        const terms = mapResults.filter(r => r.type === 'terms').map(r => r.text);
        const rfqContext = mapResults.filter(r => r.type === 'rfq').map(r => r.text).join('\n\n');
        ...
        const quoteTextsBlock = quotes.map((q, i) =>
            `=== BÁO GIÁ #${i + 1}: "${q.fileName}" ===\n${q.text}\n=== KẾT THÚC BÁO GIÁ #${i + 1} ===`
        ).join('\n\n');

        const analysisPrompt = this.buildAnalysisPrompt(quoteTextsBlock, allTerms, rfqContext, quotes.length);
            ...
            fieldsToTrace.forEach(f => {
                ...
                if (val) {
                    ...
                    for (const page of pages) {
                        const match = page.blocks.find(block => block.text.toLowerCase().includes(String(val).toLowerCase()));
                        if (match) {
